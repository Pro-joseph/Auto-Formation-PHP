...
// number of prompts by this user
$stmt = $conn->prepare("SELECT COUNT(*) FROM prompts WHERE user_id = ?");
$stmt->execute([$user_id]);
$my_prompts_count = $stmt->fetchColumn();
...
// Get 5 most recent prompts by this user
$stmt = $conn->prepare("SELECT p.id, p.title, c.name as category, u.username as author, p.created_at FROM prompts p LEFT JOIN categories c ON c.id = p.category_id LEFT JOIN users u ON u.id = p.user_id WHERE p.user_id = ? ORDER BY p.created_at DESC LIMIT 5");
$stmt->execute([$user_id]);
$my_prompts = $stmt->fetchAll();

// Get all categories with prompt count
$categories = $conn->query("SELECT c.id, c.name, c.color, c.icon, COUNT(p.id) as total FROM categories c LEFT JOIN prompts p ON p.category_id = c.id GROUP BY c.id")->fetchAll();

// Get top 5 contributors
$top_contributors = $conn->query("SELECT u.username, u.role, COUNT(p.id) as total_prompts FROM prompts p JOIN users u ON u.id = p.user_id GROUP BY u.id ORDER BY total_prompts DESC LIMIT 5")->fetchAll();
