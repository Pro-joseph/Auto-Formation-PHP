...
$stmt = $conn->prepare("SELECT p.*, c.name as category, u.username as author FROM prompts p LEFT JOIN categories c ON c.id = p.category_id LEFT JOIN users u ON u.id = p.user_id WHERE p.id = ?");
$stmt->execute([$id]);
$prompt = $stmt->fetch();
if (!$prompt) {
    header("Location: prompts.php");
...
// Get related prompts (same category, not this one)
$stmt2 = $conn->prepare("SELECT p.id, p.title, p.description, c.name as category FROM prompts p LEFT JOIN categories c ON c.id = p.category_id WHERE p.category_id = ? AND p.id != ? LIMIT 4");
