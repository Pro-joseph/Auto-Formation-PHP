...
$stmt = $conn->prepare("SELECT p.id, p.title, p.description, p.content, p.tags, p.created_at, c.name as category, c.color, u.username as author FROM prompts p LEFT JOIN categories c ON c.id = p.category_id LEFT JOIN users u ON u.id = p.user_id WHERE $whereStr ORDER BY p.created_at DESC");
$stmt->execute($params);
